...
value_orders = {}
value_orders["receiptdate1"] = list(
    pd.date_range(
        min_receiptdate1, max_receiptdate1, freq=receiptdate1_skipby
    ).strftime("%Y-%m-%d")
)
value_orders["receiptdate2"] = list(
    pd.date_range(
        min_receiptdate2, max_receiptdate2, freq=receiptdate2_skipby
    ).strftime("%Y-%m-%d")
)
...
for receiptdate1 in tqdm(value_orders["receiptdate1"], ncols=80):
    sel_receiptdate1[receiptdate1] = con.sql(
        qt_receiptdate1, params={"receiptdate1": receiptdate1}
    ).fetchone()[0]

for receiptdate2 in tqdm(value_orders["receiptdate2"], ncols=80):
    sel_receiptdate2[receiptdate2] = con.sql(
        qt_receiptdate2, params={"receiptdate2": receiptdate2}
    ).fetchone()[0]

lineitem_card = con.sql("SELECT count(*) FROM lineitem").fetchone()[0]

sel_receiptdate1 = {x: y / lineitem_card for x, y in sel_receiptdate1.items()}
sel_receiptdate2 = {x: y / lineitem_card for x, y in sel_receiptdate2.items()}

results = {
    "sel_receiptdate1": sel_receiptdate1,
    "sel_receiptdate2": sel_receiptdate2,
    "deviation_log": deviation_log,
}

with open('q12_result.json', 'w') as fp:
