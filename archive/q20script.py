...
value_orders = {}
value_orders["shipdate1"] = list(
    pd.date_range(min_shipdate1, max_shipdate1, freq=shipdate1_skipby).strftime(
        "%Y-%m-%d"
    )
)
value_orders["shipdate2"] = list(
    pd.date_range(min_shipdate2, max_shipdate2, freq=shipdate2_skipby).strftime(
        "%Y-%m-%d"
    )
)
...
for shipdate1 in tqdm(value_orders["shipdate1"], ncols=80):
    sel_shipdate1[shipdate1] = con.sql(
        qt_shipdate1, params={"shipdate1": shipdate1}
    ).fetchone()[0]

for shipdate2 in tqdm(value_orders["shipdate2"], ncols=80):
    sel_shipdate2[shipdate2] = con.sql(
        qt_shipdate2, params={"shipdate2": shipdate2}
    ).fetchone()[0]

lineitem_card = con.sql("SELECT count(*) FROM lineitem").fetchone()[0]

sel_shipdate1 = {x: y / lineitem_card for x, y in sel_shipdate1.items()}
sel_shipdate2 = {x: y / lineitem_card for x, y in sel_shipdate2.items()}

results = {
    "sel_shipdate1": sel_shipdate1,
    "sel_shipdate2": sel_shipdate2,
    "deviation_log": deviation_log,
}

with open("q20_result.json", "w") as fp:
